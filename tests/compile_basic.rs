...
fn has_decorator(func: &StmtFunctionDef, method: &str) -> bool {
    func.decorator_list.iter().any(|dec| {
        workflow_decorator_name(&dec.expression) == Some(method)
    })
}
...
fn decorator_has_kwarg_str(expr: &Expr, key: &str, value: &str) -> bool {
    ...
    call.arguments.keywords.iter().any(|kw| {
        kw.arg.as_ref().is_some_and(|a| a.as_str() == key)
            && kw.value.as_string_literal_expr().is_some_and(|s| s.value.to_str() == value)
    })
}
...
fn find_decorated_function<'a>(stmts: &'a [Stmt], decorator_method: &str) -> Option<&'a StmtFunctionDef> {
    for stmt in stmts {
        if let Stmt::FunctionDef(func) = stmt {
...
fn count_string_args(call: &ExprCall) -> usize {
    call.arguments.args.iter().filter(|a| a.is_string_literal_expr()).count()
}
...
fn compile_simple_rule() {
    ...
    assert!(has_decorator(func, "output"), "should have @workflow.output");
    assert!(has_decorator(func, "shellcmd"), "should have @workflow.shellcmd");
    assert!(has_decorator(func, "norun"), "should have @workflow.norun for non-run rules");
...
fn compile_python_passthrough() {
    ...
    assert!(matches!(&stmts[0], Stmt::Assign(_)), "first stmt should be assignment");
    assert!(matches!(&stmts[1], Stmt::Assign(_)), "second stmt should be assignment");
}
...
fn compile_run_block() {
    ...
    assert!(has_decorator(func, "run"), "should have @workflow.run decorator");
    assert!(has_decorator(func, "input"), "should have @workflow.input decorator");
    assert!(!has_decorator(func, "norun"), "should NOT have @workflow.norun with run block");

    // The function body should not be just `pass`
    let body_len = func.body.len();
    assert!(body_len >= 1, "run block body should have at least 1 statement, got {body_len}");

    // At least one statement should be an import
    let has_import = func.body.iter().any(|s| matches!(s, Stmt::Import(_)));
    assert!(has_import, "run block body should contain an import statement");
}
...
fn compile_checkpoint() {
    ...
    // Should find a function with @workflow.checkpoint(name='gather_results')
    let func = find_rule_function(&stmts, "gather_results")
        .expect("should find checkpoint 'gather_results'");
    assert!(
        has_decorator(func, "checkpoint"),
        "should have @workflow.checkpoint decorator"
    );
    assert!(has_decorator(func, "input"), "should have @workflow.input");
    assert!(has_decorator(func, "output"), "should have @workflow.output");
}
...
fn compile_global_configfile() {
    ...
    let call = find_workflow_call(&stmts, "configfile")
        .expect("should find workflow.configfile() call");
    assert!(
...
fn compile_ruleorder() {
    ...
    let call = find_workflow_call(&stmts, "ruleorder")
        .expect("should find workflow.ruleorder() call");
    assert_eq!(
...
fn compile_handler() {
    ...
    assert!(has_log_param, "handler function should accept 'log' parameter");
}
...
fn compile_mixed_workflow() {
    ...
    assert!(configfile_call.is_some(), "should have workflow.configfile()");
...
fn compile_real_workflow_fixture() {
    ...
    let assign_count = stmts.iter().filter(|s| matches!(s, Stmt::Assign(_))).count();
    assert!(
        assign_count >= 2,
    ...
    assert!(has_decorator(align, "resources"), "align should have resources");
    assert!(has_decorator(align, "log"), "align should have log");
    assert!(has_decorator(align, "conda"), "align should have conda");
    assert!(has_decorator(align, "shellcmd"), "align should have shellcmd");
}
