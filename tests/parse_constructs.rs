...
fn parse_globals_fixture() {
    ...
    assert!(
        globals >= 6,
        "expected at least 6 global directives, got {globals}"
    );
...
fn parse_handlers_fixture() {
    ...
    let handlers: Vec<_> = ast
        .body
        .iter()
        .filter_map(|s| {
            if let Statement::Handler(h) = s {
                Some(h)
            } else {
                None
            }
